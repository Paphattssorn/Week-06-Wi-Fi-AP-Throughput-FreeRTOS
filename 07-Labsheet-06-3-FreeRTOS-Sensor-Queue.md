...
| :--- | :---: | :---: | :---: |
| **`SensorCollectorTask`** | 3072 |1196 words (1196 bytes) | ปลอดภัย เหลือพื้นที่เพียงพอ ไม่เกิด Stack Overflow|
| **`NetworkCommTask`** | 4096 |2248 words (2248 bytes) |  ปลอดภัย เหลือพื้นที่เพียงพอ ไม่เกิด Stack Overflow|
...
1. เหตุใดการใช้ **FreeRTOS Queue** จึงมีความปลอดภัย (Thread-Safe) มากกว่าการใช้ตัวแปรแบบ Global ในการรับส่งข้อมูลระหว่างสอง Task?
    Queue มีกลไกจัดการลำดับและการล็อกการเข้าถึงข้อมูลในระดับ OS ช่วยป้องกันปัญหา Race Condition ข้อมูลผิดเพี้ยนจากการที่หลาย Task แย่งกันอ่านเขียนข้อมูลพร้อมกัน ซึ่งตัวแปร Global ไม่มีกลไกป้องกันนี้
2. ค่า **Stack High Water Mark** มีประโยชน์อย่างไรในการตรวจวินิจฉัยปัญหาบั๊กในระบบเรียลไทม์ (RTOS)?
    ใช้ตรวจสอบพื้นที่ Stack ที่เหลือน้อยที่สุด ที่ Task นั้นเคยใช้งาน หากค่านี้มีตัวเลขเข้าใกล้ 0 แสดงว่าระบบเสี่ยงต่อการเกิดเมมโมรีล้นจนบอร์ดแครช ช่วยให้นักพัฒนาสามารถคำนวณและปรับลดเพิ่มขนาด Stack ของแต่ละ Task ได้
3. หาก `vSensorTask` ส่งข้อมูลเร็วมาก (เช่น ทุก 10ms) แต่ `vNetworkTask` ส่งข้อมูลออก Wi-Fi ได้ช้า (เช่น ใช้เวลา 500ms) จะเกิดอะไรขึ้นกับ Queue และระบบจะรับมืออย่างไร?
    คิวจะเข้าสู่สถานะ เต็ม Queueอย่างเร็ว ระบบจะรับมือตามค่า Timeout xTicksToWait ที่ตั้งไว้ในฟังก์ชันส่งข้อมูลลงคิว
